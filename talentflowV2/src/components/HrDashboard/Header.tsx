...
const Header: React.FC = () => {
  ...
  //close chat dropdown when clicked outside
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
          ...
          <div className="flex items-center sp md:space-x-6">
            ...
            <div ref={chatRef} className=" relative">
              <div className="hidden  sm:flex items-center space-x-4">
                <button
                  onClick={toggleChat}
                  className="cursor-pointer text-gray-700 hover:text-emerald-600 font-medium transition-colors duration-200"
                >
                  <svg
                    className="w-6 h-6"
                    fill="none"
              ...
              {/* chat modal */}
              {isChatOpen && (
                <div className="absolute -right-1/2 mt-6 w-96  rounded-lg shadow-lg border border-gray-200 py-2 z-50 bg-white">
                  <ChatModal />
                </div>
              )}
            </div>
